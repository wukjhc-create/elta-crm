...
import type { CreateRoomCalculationInput, ProjectCalculationInput } from '@/types/calculation-intelligence.types'
import type { LoadEntry, ElectricalRoomInput, ElectricalProjectResult } from '@/types/electrical.types'
import type { ProjectEstimationInput, ProjectEstimationResult } from '@/types/project-estimation.types'
...
import { analyzeMargins, CUSTOMER_TIERS, type CustomerTier } from '@/lib/services/price-engine'
...
export async function createProjectEstimation(
  ...
  try {
    ...
    // Load calculation intelligence data (reference tables - capped for safety)
    const [installTypesRes, roomTemplatesRes, componentTimeRes] = await Promise.all([
      supabase.from('installation_types').select('*').eq('is_active', true).limit(200),
      supabase.from('room_templates').select('*').eq('is_active', true).limit(200),
      supabase.from('component_time_intelligence').select('*').limit(500),
    ])
    ...
    try {
      ...
      logger.warn('Electrical calculation failed (continuing without)', { error: elErr, action: 'createProjectEstimation' })
    }
