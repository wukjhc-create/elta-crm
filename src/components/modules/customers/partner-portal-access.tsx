...
interface PartnerPortalAccessProps {
  customerId: string
...
export function PartnerPortalAccess({
  ...
  const handleCreateToken = async () => {
    ...
    try {
      const result = await createPartnerToken({
        partner_customer_id: customerId,
        email: customerEmail,
      })

      if (result.success) {
        toast.success('Partner-adgang oprettet')
      } else {
        toast.error('Kunne ikke oprette partner-adgang', result.error)
        return
      }
  ...
  const handleDeactivate = async (tokenId: string) => {
    if (!confirm('Er du sikker på at du vil deaktivere denne partner-adgang?')) {
      return
    }
    ...
    try {
      const result = await deactivatePartnerToken(tokenId)
    ...
    <div className="bg-white rounded-lg border p-6">
      ...
      {activeTokens.length === 0 ? (
        <div className="text-center py-6">
          <Handshake className="w-10 h-10 mx-auto text-gray-300 mb-2" />
          <p className="text-gray-500 text-sm">
            Ingen aktiv partner-adgang
          </p>
          <p className="text-gray-400 text-xs mt-1">
            Opret en adgang så partneren kan se alle sager hvor de er betaler og
            hente dokumentation
          </p>
        </div>
      ) : (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <a
                    href={getPartnerUrl(token.token)}
                    target="_blank"
